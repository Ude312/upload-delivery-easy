...
**PII and confidential data exposure:** Delivery notes sent to the app are forwarded to OpenAI's API servers. These documents may contain supplier names, internal order numbers, prices, and contact details — all potentially confidential under GDPR or trade secret law. For this academic prototype, this is an accepted limit. For production use at KWS SAAT SE, the mitigation would be to switch to Azure OpenAI (EU data residency, GDPR-compliant) or a locally-hosted model with no external data transfer, and to implement a PII scrubbing step before the API call.
